...
- 为什么会有一个静态的?因为有些操作是在`static`中操作的,你可能会说,那我在static函数中new一个`LocalizationManager `也可以吖,是的,没问题,但是`静态的LocalizationManager`是用来解决静态公用多语言的,我一般是在[InitializeOnRun](#1InitializeOnRun%20特性)运行时将自己的静态语言进行加载,如编辑器的Title
